...
export default function BusquedaFacturas({proporcional}) {

    const dispatch = useDispatch()
        
    const [cantidad_mostrar, setCantidadMostrar] = useState(10)
    const [buscar_factura, setBuscarFactura] = useState ('')

    const mostrar_facturas_por_cantidad = () => {
        dispatch(facturasdata(facturasCosntants(0, buscar_factura === '' ? 0 : buscar_factura, 0, 0, 0, parseInt(cantidad_mostrar), {}, false).get_facturas_paciente_search))
    }

    const buscar_facturas = () => {
        if (buscar_factura !== ''){
            dispatch(facturasdata(facturasCosntants(0, buscar_factura, 0, 0, 0, 16, {}, false).get_facturas_paciente_search))
        }
    }

    return (
        <div className='d-flex justify-content-between' style={{width: '100%', height: 'auto', padding: 22 / proporcional, background: 'white'}}>
            <div className='d-flex justify-content-start' style={{width: '50%', height: 'auto'}}>
                ...
                <select 
                    ...
                    onChange={(event) => {setCantidadMostrar(event.target.value); mostrar_facturas_por_cantidad()}}
            ...
            <div className='d-flex justify-content-end' style={{width: '50%', height: 45 / proporcional}}>
                ...
                <input 
                    ...
                    value={buscar_factura}
                    onChange={(event) => setBuscarFactura(event.target.value)}
